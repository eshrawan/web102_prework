...
function addGamesToPage(games) {
    ...
    for(var i = 0; i < games.length; i++) {
        ...
        game_div.innerHTML = `<img class="game-img" src="${games[i].img}" />
        <h2>${games[i].name}</h2>
        <p>${games[i].description}</p>
        <span>Backers: ${games[i].backers}</span>`;
        // append the game to the games-container
        // Source: https://stackoverflow.com/questions/44440833/how-to-append-a-div-to-a-container
...
const totalPledges = GAMES_JSON.reduce((accumulator, currentValue) => accumulator + currentValue.pledged , 0);
...
submitBtn.addEventListener("click", showSearchGame);
...
let countUnfunded = unfundedGames.length;
let countFunded = GAMES_JSON.filter(game => game.pledged >= game.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
let displayStr = `${countUnfunded>0 ? `<p>A total of ${totalPledges.toLocaleString()} has been raised to fund ${countFunded} funded games. 
Currently ${countUnfunded} remain unfunded. We need your help
to fund these amazing games!</p>` : `<p>All games are backed and funded. Thank you for your support!</p>`}`;
...
const[topgame, secondgame, ...allGames] = sortedGames;
